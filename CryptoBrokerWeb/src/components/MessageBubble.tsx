  ...
  return (
    <article className={`message-row ${own ? "own" : ""} ${selected ? "selected" : ""}`} onClick={onSelect}>
      ...
      <div className="message-bubble">
        {message.forwardedFrom && <span className="forwarded">Переслано от {message.forwardedFrom}</span>}
        ...
        {message.attachments.map((attachment) => (
          <div className={`attachment attachment-${attachment.type}`} key={attachment.id}>
            ...
            <span>{attachment.size || attachment.duration || attachment.metadata?.place || "предпросмотр"}</span>
          </div>
        ))}
        <p>{message.text}</p>
